 ...
 * @param {RequireContext} modulesFiles 由webpack提供的require.context方法返回的上下文, vite中使用import.meta.globEager('./modules/*.js')
...
export function flatArray(arr) {
  ...
  arr.forEach((item) => {
    ...
    if (item.children && item.children.length !== 0) {
      flatArr.push(...flatArray(item.children))
    }
  })
  return flatArr
}
